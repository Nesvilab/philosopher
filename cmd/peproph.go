...
import (
	...
	"github.com/prvst/philosopher-source/lib/ext/peptideprophet"
...
func init() {
	...
	peprophCmd.Flags().BoolVarP(&pep.Zero, "zero", "", false, "report results with minimum probability 0 (default 0.05)")
	peprophCmd.Flags().BoolVarP(&pep.Accmass, "accmass", "", false, "use Accurate Mass model binning")
	peprophCmd.Flags().StringVarP(&pep.Clevel, "clevel", "", "", "set Conservative Level in neg_stdev from the neg_mean, low numbers are less conservative, high numbers are more conservative")
	...
	peprophCmd.Flags().StringVarP(&pep.Masswidth, "masswidth", "", "5.0", "model mass width")
	peprophCmd.Flags().BoolVarP(&pep.Pi, "pi", "", false, "enable peptide pI model")
	peprophCmd.Flags().StringVarP(&pep.Minpintt, "minpintt", "", "", "minimum number of NTT in a peptide used for positive pI model (default 2)")
	peprophCmd.Flags().StringVarP(&pep.Minpiprob, "minpiprob", "", "", "minimum probability after first pass of a peptide used for positive pI model (default 0.9)")
	peprophCmd.Flags().BoolVarP(&pep.Rt, "rt", "", false, "enable peptide RT model")
	peprophCmd.Flags().StringVarP(&pep.Minrtntt, "minrtntt", "", "", "minimum number of NTT in a peptide used for positive RT model")
	peprophCmd.Flags().StringVarP(&pep.Minrtprob, "minrtprob", "", "", "minimum probability after first pass of a peptide used for positive RT model")
	...
	peprophCmd.Flags().StringVarP(&pep.Minprob, "minprob", "", "", "report results with minimum probability <number> (default 0.05)")
	...
	peprophCmd.Flags().StringVarP(&pep.Ignorechg, "ignorechg", "", "", "can be used multiple times to specify all charge states to exclude from modeling")
