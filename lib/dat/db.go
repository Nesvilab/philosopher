...
func getProteinExistence(header, class string, verb bool) (match string) {
	...
	if reg == nil || len(reg) <= 1 {
		if verb {
			msg.ParsingFASTA(errors.New("could not detectd the protein existence"), "warning")
		}
...
func getGeneName(header, class string, verb bool) (match string) {
	...
	if reg == nil || len(reg) <= 1 {
		...
		if verb {
			msg.ParsingFASTA(errors.New("could not detectd the gene name"), "warning")
		}
...
func getOrganism(header, class string, verb bool) (match string) {
	...
	if reg == nil || len(reg) <= 1 {
		...
		if verb {
			msg.ParsingFASTA(errors.New("could not detectd the organism name"), "warning")
		}
...
func getProteinName(header, class string, verb bool) (match string) {
	...
	if reg == nil || len(reg) <= 1 {
		...
		if verb {
			msg.ParsingFASTA(errors.New("could not detectd the protein name"), "warning")
		}
...
func getEntryName(header, class string, verb bool) (match string) {
	...
	case "uniref":
		r = regexp.MustCompile(`(UniRef\w+)`)
	case "tair":
		r = regexp.MustCompile(`(AT\d\w\d{5}\.?(\d{1,2})?)`)
	case "nextprot":
	...
	if reg == nil || len(reg) < 1 {
		...
		if verb {
			msg.ParsingFASTA(errors.New("could not detectd the entry name"), "warning")
		}
...
func getID(header, class string, verb bool) (match string) {
	...
	if reg == nil || len(reg) <= 1 {
		...
		if verb {
			msg.ParsingFASTA(errors.New("Could not detectd the entry ID"), "warning")
		}
