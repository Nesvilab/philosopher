...
func writeDatabase(d met.Database, text string) string {
	...
	if d.Crap == true {
		text = fmt.Sprintf("%s 153 common contaminants were also added to the database.", text)
	}

	text = fmt.Sprintf("%s Decoy sequences were generated by reversing the protein sequences and adding the %s prefix to their headers.", text, d.Tag)
